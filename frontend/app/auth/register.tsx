...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      setSuccess(true);
      // Redirect to login after successful registration and prefill email
      setTimeout(() => {
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          <View style={[styles.card, { backgroundColor: colors.card, shadowColor: colors.shadow }]}> 
            <View style={styles.headerRow}>
              <Text style={[styles.title, { color: colors.text }]}>{t('settings.register')}</Text>
            ...
            {/* Display name */}
            <View style={[styles.inputGroup]}> 
              <Text style={[styles.label, { color: colors.sub }]}>{t('auth.displayName')}</Text>
              <View style={[styles.inputWrapper, { borderColor: colors.inputBorder, backgroundColor: colors.inputBg }]}> 
                <MaterialIcons name="badge" size={20} color={colors.icon} style={{ marginRight: 8 }} />
                <TextInput
                  style={[styles.input, { color: colors.inputText }]}
              ...
              {!!displayName && !isNameValid && (
                <Text style={[styles.validation, { color: colors.danger }]}>At least 2 characters</Text>
              )}
            </View>
            ...
            <View style={[styles.inputGroup]}> 
              ...
              <View style={[styles.inputWrapper, { borderColor: colors.inputBorder, backgroundColor: colors.inputBg }]}> 
                ...
                <TextInput
                  ...
                  returnKeyType="go"
                  onSubmitEditing={() => { if (isFormValid && !loading) handleRegister(); }}
                />
            ...
            {error && <Text style={[styles.error, { color: colors.danger }]}>{t('auth.registrationError', { error })}</Text>}
            {success && <Text style={[styles.success, { color: colors.success }]}>{t('auth.registrationSuccess')}</Text>}
            ...
            <Text style={[styles.helperText, { color: colors.sub }]}>{t('auth.alreadyHaveAccount')}</Text>
            <Pressable
              onPress={() => router.replace('/auth/login')}
              style={({ pressed }) => [styles.secondaryBtn, { opacity: pressed ? 0.85 : 1, borderColor: colors.inputBorder }]}
            >
              <Text style={[styles.secondaryBtnText, { color: colors.text }]}>{t('settings.login')}</Text>
            </Pressable>
          </View>
        </ScrollView>
